#!/usr/bin/env bash
# Первичная настройка сервера FirstByte для iqos-store.ru
# Запуск на сервере под root:
#   export GITHUB_REPO="https://github.com/ВАШ_ЛОГИН/iqos-store.git"
#   bash bootstrap_server.sh

set -euo pipefail

APP_USER="lilstore"
APP_DIR="/home/${APP_USER}/iqos-store"
DOMAIN="iqos-store.ru"
SERVICE="iqos-store"
...
if [[ -n "$GITHUB_REPO" ]] && [[ ! -d "$APP_DIR/.git" ]]; then
  ...
  echo "или скопируйте файлы через scp."
fi
...
if [[ ! -f "$APP_DIR/config.py" ]] && [[ -f "$APP_DIR/config.example.py" ]]; then
  ...
  echo "Создан config.py из шаблона — отредактируйте: nano $APP_DIR/config.py"
fi
...
echo "4. После SSL: cp $APP_DIR/deploy/nginx-iqos-store.conf /etc/nginx/sites-available/iqos-store && nginx -t && systemctl reload nginx"

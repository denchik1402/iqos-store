...
# Подстроки в запросе → каноническая латиница (длинные первыми)
...
# Если в запросе встречается ключ — добавить связанные термины
_QUERY_EXPANSIONS = {
...
def expand_search_terms(query: str) -> list[str]:
    """Уникальные поисковые фразы: оригинал, транслит, синонимы."""
    raw = (query or '').strip()
    if not raw:
        return []
...
def product_search_filter(query: str, Product, *, in_stock_only: bool = False):
    """Фильтр SQLAlchemy: name, description, meta_keywords, model, color."""
